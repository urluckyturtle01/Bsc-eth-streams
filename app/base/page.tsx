...
import { useBaseKafkaConsumer } from '../hooks/useBaseKafkaConsumer'
import FeedHeader from '../components/FeedHeader'

const BasePage: React.FC = () => {
  const { events, isConnected, error, connect, disconnect, clearEvents } = useBaseKafkaConsumer()
          ...
          {filteredEvents.length === 0 ? (
            <div className="text-center py-20">
              ...
              filteredEvents.map((event, index) => {
                ...
                const hasPnl = event.pnlMint7d !== null && event.pnlMint7d !== undefined
                
                const eventId = `${event.transactionId}-${event.baseMint}-${event.receivedAt}-${index}`
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            window.open(`https://basescan.org/address/${event.walletAddress}`, '_blank')
                          }}
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            window.open(`https://basescan.org/tx/${event.transactionId}`, '_blank')
                          }}
                            ...
                            {event.poolAddress ? (
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    window.open(`https://basescan.org/address/${event.poolAddress}`, '_blank')
                                  }}
                                  className="text-blue-400 hover:text-blue-300 font-mono text-[11px]"
                                >
